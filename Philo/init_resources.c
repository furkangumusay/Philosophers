...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/16 00:36:54 by fgumusay          #+#    #+#             */
/*   Updated: 2024/11/16 00:36:54 by fgumusay         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int    init_mutexes(t_sim *sim)
{
    ...
    while (++i < sim->philo_count)
        if (pthread_mutex_init(&sim->forks_mutex[i], NULL)) //++i
...
int     init_philosophers(t_sim *sim)
{
    ...
    while (++i < sim->philo_count)
    {
        ...
        sim->philo[i].right_fork_id = (i + 1) % sim->philo_count;
    }
    return (0);
}
